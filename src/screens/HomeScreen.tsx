...
export default function HomeScreen({ navigation }) {
  const { user, logout, loading: authLoading, authenticated } = useContext(AuthContext);
    ...
    try {
      ...
      try {
        if (user && treinosData && Array.isArray(treinosData)) {
          filteredTreinos = treinosData.filter((t: any) => {
            if (t.usuarioId !== undefined) return t.usuarioId === user.id;
            if (t.usuario && t.usuario.id !== undefined) return t.usuario.id === user.id;
            return true;
          });
        }
      } catch (e) {
